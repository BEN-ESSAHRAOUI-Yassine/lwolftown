<div class="flex items-center justify-center min-h-screen">
    <div class="bg-gray-900/50 border border-gray-700 rounded-lg p-8 w-full max-w-md">
        <h2 class="font-cinzel text-xl text-white mb-6 text-center">{{ __('ui.join_room') }}</h2>
        <form wire:submit="submit">
            <div class="mb-4">
                <label for="code" class="block text-sm text-gray-400 mb-1">{{ __('ui.room_code') }}</label>
                <input
                    type="text"
                    id="code"
                    wire:model="code"
                    maxlength="6"
                    required
                    class="w-full bg-gray-800 border border-gray-600 text-white px-3 py-2 rounded focus:outline-none focus:border-amber-500 uppercase tracking-widest font-cinzel text-center text-lg"
                />
                @error('code')
