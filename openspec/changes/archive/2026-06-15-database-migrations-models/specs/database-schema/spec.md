...
The system SHALL create a roles table with columns for key, faction, night_order, abilities JSON, and win_condition.

#### Scenario: Roles table structure
- **WHEN** the roles migration runs
- **THEN** the table has columns: id (bigint PK), key (varchar, unique), faction (varchar), night_order (int nullable), abilities (json nullable), win_condition (varchar), created_at, updated_at
